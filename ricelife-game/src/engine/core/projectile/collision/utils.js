...
export function getSegmentCollision (position, projected, segmentStart, segmentEnd, radius, clockwise = true, movementVector = undefined) {
    if (!position?.isVector) throw new Error(`Invalid parameter - expected Vector, got ${typeString(position)}`);
    if (!projected?.isVector) throw new Error(`Invalid parameter - expected Vector, got ${typeString(projected)}`);
    if (!segmentStart?.isVector) throw new Error(`Invalid parameter - expected Vector, got ${typeString(segmentStart)}`);
    if (!segmentEnd?.isVector) throw new Error(`Invalid parameter - expected Vector, got ${typeString(segmentEnd)}`);
    if (!Number.isFinite(radius)) throw new Error(`Invalid parameter - expected Number, got ${typeString(radius)}`);
