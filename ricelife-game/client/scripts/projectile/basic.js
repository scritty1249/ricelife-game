...
export class Spreader extends BasicShot {
    ...
    constructor (origin, angle, power = 1, resolution = 1) {
        ...
        this.blast._shapes.push(new Circle(new Vector(-this.config.blastRadius * 1.75, 0), this.config.blastRadius));
        this.blast._shapes.push(new Circle(new Vector(this.config.blastRadius * 1.75, 0), this.config.blastRadius));
    }
}
...
export class Flower extends BasicShot {
    ...
    constructor (origin, angle, power = 1, resolution = 1) {
        ...
        this.blast._shapes.push(...Array.from([0, 360/7, 720/7, 1080/7, 1440/7, 1800/7, 2160/7], (angle) => {
            ...
            return new Circle(new Vector(Math.cos(rad), Math.sin(rad)).mul(this.config.radius + (this.config.blastRadius * 1.75)), this.config.blastRadius)}));
    }
}