...
import { WorkerPool, PoolManager } from "/engine/workers/Core.js";
...
export class Round extends Phase {
    static SETTINGS = {
        // time in seconds before the "busy" screen pops up while tracing shots
        BUSY_SECONDS_THRESHOLD: 1.5,
        // (seconds) will trigger a landing early if timeout is exceeded
        // however a landing will only be traced within this time frame so early landings shouldn't be happening... -KT
        SHOT_TRACE_LIMIT: 30,
        AIM_SENSITIVITY: Math.PI / 180,
        POWER_SENSITIVITY: .005,
        PAN_SENSITIVITY: 5,
        MOVE_SPEED: 1,
        // scale to multiply player tank size by to use as target viewbox size when tracking player
        PLAYER_SCREEN_TRACKING_SCALE: 30,
        // number of milliseconds before game waits for player input to play shot animation. If loading takes less time, shot animation is played automatically
        LOADING_PAUSE_THRESHOLD: 4 * 1000,
    ...
    #loadPromise;
    constructor (mainController, lobby, terrain) {
        super(mainController);
        this.#init(lobby, terrain);
