import { InputListener, AppCanvas, WorkerController, FrameCounter, Interval } from "./controller/controller.js";
...
async function fireProjectile (shot, state, config) { // [!} laziness
    ...
    const launchOrigin = state.terrain.isIntersecting(player.tank.barrelPosition)
        ? player.tank.relativePosition
        : player.tank.barrelPosition;
...
function generateMuzzleFlash (state, config, width = 600) {
    ...
    const ani = new Animation(player.tank.barrelPosition, ss, ss.framerate);
    ani.speed = 2.3;
...
function drawDebugOverlay (state, config) {
    ...
    drawCircle(cursor, player.tank.barrelPosition);
    ...
    if ((state.input.pointer.isDragging
        ...
        drawText(cursor, position, `${position.toString()}, (${wrapDeg(rad2deg(player.tank.barrelPosition.angle(position)).toFixed(0) - 90)})`, c.toString());
        c.a = .5;
        drawLine(cursor, player.tank.barrelPosition, position, 2, c.toString());
    }
    if (state.input.keyboard.keyActive("debug+")) {
        {
            // draw Y-axis positioning raycasters
            const ray = new Ray(new Vector(player.tank.position.x, 0), Vector.fromAngle(Math.PI/2), config.display.size.y - 20);
        ...
        const mode = state.input.keyboard.keyActive("shot1")
            ...
            : state.input.keyboard.keyActive("shot2")
                ...
                : 0; // show all hits
        const hits = state.terrain.raycast(new Ray(player.tank.barrelPosition, position))
            .filter(({hole}) => 
                (mode === 0)
                || (mode === 1 && hole)
                || (mode === 2 && !hole));
