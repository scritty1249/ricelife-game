...
export class HitPoints extends Hashable {
    ...
    static register () {
        if (!Number.isFinite(this.TYPE)) throw new Error(`[${typeString(this)}]: Unable to register undefined HP type`);
        HitPoints.TYPES.set(this.TYPE, this);
        console.debug(`HP type registered: ${this.name}`);
    }
