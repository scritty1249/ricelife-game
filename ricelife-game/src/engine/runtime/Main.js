...
export class Main extends Loop {
    ...
    #initAssetTable () {
        ...
        AssetTable.hideInctiveBtn = [AssetType.Image, undefined, "/assets/interface/buttons/hide-button-inactive.png"];
        // Audio
    ...
    async loadRoundPhase (lobbyData, terrainData, distributePlayers = false) {
        const { Round } = await import("./phases/Round.js");
        const phase = new Round(this, this.clientID, lobbyData, terrainData, distributePlayers);
        await phase.onload;
    ...
    async tick () {
        if (this.state === this.constructor.STATES.Ready) {
            ...
            if (this.ActivePhase?.state === this.constructor.STATES.Ready) {
                if (this.TickInterval.ready) await this.ActivePhase?.tick?.(this.TickInterval.lastDelta);
