...
import { Round, Join, Create, Loadout } from "./phases/Core.js";
import { initTerrain, initLobby } from "./utils.js";


export class Main extends Loop {
    static SETTINGS = {
        MAX_VIEWBOX_SCALE: 5, // [!] remove this property
...
export class Main extends Loop {
    ...
    async newRound (decodedLobby, decodedTerrain) {
        const terrain = initTerrain(decodedTerrain);
        const lobby = initLobby(decodedLobby);
        const phase = new Round(this, lobby, terrain);
        this.#registerPhase(phase)
            .then((id) => this.switchPhase(id, false))
